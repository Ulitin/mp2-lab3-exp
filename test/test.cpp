...
TEST(EXPression, test_on_abc_2)
{
	...
	ASSERT_TRUE(3 == input.count());
}

TEST(EXPression, test_on_abc_3)
{
	expression input("(|-5^(-1)*5|-|-2|)");
	ASSERT_TRUE(-1 == input.count());
}
